...
describe('scenePanelFromToolCall', () => {
  ...
  it('工具名映射与后端 tools_ui.go / tools_ops.go 对齐', () => {
    // ui_show_operation 在 engine/tools_ops.go(部署操作卡片),其余在 tools_ui.go
    expect(Object.keys(SCENE_PANEL_TOOLS).sort()).toEqual(
      ['ui_show_form', 'ui_show_grid', 'ui_show_list', 'ui_show_operation'],
    );
