/**
 * vrm/legacySceneBuilders.ts — 旧版硬编码 builder（保留用于平滑迁移）
 *
 * Phase 1 期间：新 sceneBuilders.ts（config-driven）已经写好，useVrmScene 还在用旧 build* 函数。
 * 切到新版只需要把 useVrmScene 里 buildSceneByName 换成 buildScene(config)。
 * 删除此文件是 Phase 1.6 的事。
 */
...
function makeStageFloor(THREE_NS: typeof THREE, radius: number, color: number) {
  const m = new THREE_NS.Mesh(
    new THREE_NS.CircleGeometry(radius, 64),
    new THREE_NS.MeshStandardMaterial({ color, roughness: 0.25, metalness: 0.85 }),
  );
...
function makeParticles(THREE_NS: typeof THREE, count = 180, area = 20) {
  ...
  const palette = [new THREE_NS.Color(0xff4fd8), new THREE_NS.Color(0x4fd8ff),
...
export function buildNeon(THREE_NS: typeof THREE, sceneGroup: THREE.Group): SceneHandle {
  ...
  [{ c: 0xff0066, p: [-2.5, 3, 2.5] as [number, number, number] },
   ...
   { c: 0x00ff66, p: [2, 3, -1.5] as [number, number, number] }].forEach(({ c, p }) => {
  ...
  for (const [x, z] of treePositions) {
    ...
    for (let i = 0; i < 3; i++) {
      const leaf = new THREE_NS.Mesh(
        ...
        new THREE_NS.MeshStandardMaterial({ color: leafColors[i % leafColors.length], roughness: 0.8 }),
      );
