...
/**
 * 工作室通用布局(参考下载/1.png)
 * 左侧:引导式聊天面板(通过对话创建实体,也可切换手动表单)
 * 右侧:上半为实体列表(增/改/删/选中),下半为节点图画布(StudioGraph)
    ...
    <Box sx={{ display: 'flex', gap: 2, height: '100%', minHeight: 0 }}>
      ...
      >
        <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          >
            <ToggleButton value="chat" sx={{ textTransform: 'none', px: 1.5 }}>对话创建</ToggleButton>
            <ToggleButton value="manual" sx={{ textTransform: 'none', px: 1.5 }}>手动添加</ToggleButton>
          </ToggleButtonGroup>
        </Box>
        ...
        {mode === 'chat' ? (
          ...
          <Box sx={{ flex: 1, overflowY: 'auto', p: 2 }}>{manualForm}</Box>
        )}
      </Box>
