...
export default function AgentStudio({ editingId = null }: { editingId?: number | null }) {
  ...
  const loadIntoDraft = (a: Agent) => {
    ...
    setModel(a.model ?? 'deepseek-chat')
  ...
  const manualForm = (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      ...
      <Box sx={{ display: 'flex', gap: 1 }}>
        <TextField size="small" label="角色" value={role} onChange={(e) => { setRole(e.target.value); setDirty(true) }} sx={{ flex: 1 }} />
        <TextField size="small" label="模型" value={model} onChange={(e) => { setModel(e.target.value); setDirty(true) }} sx={{ flex: 1 }} />
      </Box>
