/**
 * MD5(UTF-8 字符串 → 32 位小写十六进制)。
 * 只给本地流解析的请求签名用(B 站 wbi 的 w_rid = md5(参数串 + mixin key))——
 * Web Crypto 不提供 MD5,为这一个函数引一个包不值当。
 */
...
export function md5(input: string): string {
  ...
  const out = core(words, bytes.length * 8);
  let hex = '';
  for (let i = 0; i < 16; i++) hex += ((out[i >> 2] >>> ((i % 4) * 8)) & 0xff).toString(16).padStart(2, '0');
  return hex;
}
