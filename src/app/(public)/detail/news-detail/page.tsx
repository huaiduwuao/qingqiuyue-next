...
function NewsDetailContent() {
  ...
  const query = useQuery({
    ...
    queryFn: () => contentDetail({ id: id! }).then((r) => r as Partial<News>),
    enabled: !!id,
  });
      ...
      <DetailHeader
        ...
        rightActions={
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            ...
            <ShareButtons variant="icon" contentType="news" contentId={Number(id)} title={query.data?.title ?? 'news-detail 详情'} url={typeof window !== 'undefined' ? window.location.href : ''} />
          </Box>
        }
      />
        ...
        {(data) => (
          <Container maxWidth="md" sx={{ py: 3 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <CollectButton contentId={id!} contentType="news" variant="button" compact />
              </Box>
            </Box>
