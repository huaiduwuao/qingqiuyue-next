...
// 跟 VSHOW(选集列表,每集是视频)的关键差异:每章是长文本(纯文字)
// 而不是视频文件。视频走 /file/upload,文字直接拼到 content JSON。
//
      ...
      return {
        ...
        content: JSON.stringify({
          ...
          chapters: chapters.map((c, i) => ({
            ...
            // 第一版全量 body 存 JSON(单章 ≤ 10k 字,可控);
            // 后续后端 chapter 独立表落地后改用章节 ID 引用
            body: c.body,
          })),
        }),
        contentType: 'NOVEL',
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Typography sx={{ fontSize: 12, color: 'text.secondary', mt: 0.25 }}>
            章节连载 · 单本 100k+ · 每章 ≤ {MAX_CHAPTER_BODY.toLocaleString()} 字
          </Typography>
        </Box>
        <Box sx={{ textAlign: 'right' }}>
          ...
          <Typography sx={{ fontSize: 18, fontWeight: 700, color: 'primary.main', fontVariantNumeric: 'tabular-nums' }}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          >
            {f.isPending
              ? '提交中...'
              : `提交小说 (${totalChapterCount} 章 · ${totalWordCount.toLocaleString()} 字)`}
          </Button>
        </Box>
