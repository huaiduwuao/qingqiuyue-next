...
const LIST_KEY = ['content', 'article'];
...
export default function ArticleContentPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { showMessage('删除成功'); invalidate(); actionRef.current?.reload(); },
    onError: (err: any) => showMessage(err.message || '删除失败', 'error'),
  });

  const processMutation = useMutation({
    mutationFn: (vals: any) => process(vals),
    onSuccess: () => { showMessage('操作成功'); invalidate(); actionRef.current?.reload(); },
  ...
  const handleStatusChange = (record: ArticleItem, status: string | null, moduleContentStatus: string | null) => {
  ...
  const handleSearchToggle = (record: ArticleItem, checked: boolean) => {
    processMutation.mutate({
      ids: [record.id as number],
...
function OperationModal({ open, onClose, record }: OperationModalProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: (vals: ArticleItem) => save(vals),
    onSuccess: () => { onClose(); qc.invalidateQueries({ queryKey: LIST_KEY }); },
    onError: (err: any) => console.error(err),
  });

  const updateMutation = useMutation({
    mutationFn: (vals: ArticleItem) => update(vals),
    onSuccess: () => { onClose(); qc.invalidateQueries({ queryKey: LIST_KEY }); },
    onError: (err: any) => console.error(err),
  });
  ...
  const handleSubmit = () => {
    if (record?.id) {
      updateMutation.mutate({ ...record, ...formData } as ArticleItem);
    } else {
      saveMutation.mutate(formData as ArticleItem);
    }
  };
