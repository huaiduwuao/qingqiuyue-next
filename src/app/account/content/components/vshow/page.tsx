...
export default function VshowContentPage() {
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth><DialogTitle>{selectedRecord?.id ? '编辑综艺' : '新建综艺'}</DialogTitle><DialogContent><Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}><TextField label="标题" value={formValues.title} onChange={handleFormChange('title')} fullWidth /><TextField label="主持人" value={formValues.host} onChange={handleFormChange('host')} fullWidth /><TextField label="简介" value={formValues.info} onChange={handleFormChange('info')} fullWidth multiline rows={3} /></Box></DialogContent><DialogActions><Button onClick={() => setWriteVisible(false)}>取消</Button><Button variant="contained" onClick={handleSubmit} disabled={isSubmitting}>提交</Button></DialogActions></Dialog>
