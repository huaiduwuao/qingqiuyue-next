...
import { page, process, remove, save, update } from '@/apis/content-animation';
...
const LIST_KEY = ['content', 'animation'];

export default function AnimationContentPage() {
  ...
  const columns: GridColDef[] = [
    {
      ...
      width: 100,
      renderCell: (params) => {
        return STATUS_MAP[params.value] || params.value;
      },
    },
    {
    ...
    { field: 'title', headerName: '标题', width: 150 },
    {
      ...
      width: 300,
      renderCell: (params) => (
        <Typography noWrap sx={{ maxWidth: 280 }}>{params.value}</Typography>
      ),
    },
    {
      field: 'updateTime',
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Typography variant="h5" sx={{ mb: 2 }}>动漫管理</Typography>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await page('animation', { ...params, pageNumber: params.pageNumber });
          return {
            data: {
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑动漫' : '新建动漫'}</DialogTitle>
        <DialogContent>
