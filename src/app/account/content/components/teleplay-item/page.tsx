...
import { page, remove, save, update } from '@/apis/content-teleplay-item';
...
const LIST_KEY = ['content', 'teleplay-item'];

export default function TeleplayItemContentPage({ detail, handleClose }: Props) {
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h5">{detail?.title || '电视剧'} - 剧集管理</Typography>
        <Button onClick={handleClose}>关闭</Button>
      </Box>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await page({ ...params, teleplayId: detail?.id });
