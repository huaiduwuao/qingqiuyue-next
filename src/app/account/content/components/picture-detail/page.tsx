...
import { page, remove, save, update } from '@/apis/content-picture-detail';
...
const LIST_KEY = ['content', 'picture-detail'];

export default function PictureDetailContentPage({ detail, handleClose }: Props) {
  ...
  const handleEdit = (record: any) => {
    ...
    setFormValues({ name: record?.name || '', url: record?.url || '' });
  ...
  const columns: GridColDef[] = [
    { field: 'name', headerName: '名称', width: 200 },
    {
      ...
      headerName: '封面',
      width: 120,
      renderCell: (params) => params.value ? <img src={params.value} alt="cover" style={{ width: 80, height: 80, objectFit: 'cover' }} /> : null,
    },
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h5">{detail?.title || '图集'} - 图片管理</Typography>
        <Button onClick={handleClose}>关闭</Button>
      </Box>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await page({ ...params, album: detail?.id });
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑图片' : '新建图片'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            <TextField label="名称" value={formValues.name} onChange={handleFormChange('name')} fullWidth />
            <TextField label="封面地址" value={formValues.url} onChange={handleFormChange('url')} fullWidth />
          </Box>
        </DialogContent>
        <DialogActions>
