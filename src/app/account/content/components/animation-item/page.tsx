...
import { page, remove, save, update } from '@/apis/content-animation-item';
...
const LIST_KEY = ['content', 'animation-item'];

export default function AnimationItemContentPage({ detail, handleClose }: Props) {
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h5">{detail?.title || '动画'} - 剧集管理</Typography>
        <Button onClick={handleClose}>关闭</Button>
      </Box>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await page({ ...params, animationId: detail?.id });
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑剧集' : '新建剧集'}</DialogTitle>
        <DialogContent>
