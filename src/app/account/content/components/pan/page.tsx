...
import { page, process, remove, save, update } from '@/apis/content-pan';
...
const LIST_KEY = ['content', 'pan'];

export default function PanContentPage() {
  const qc = useQueryClient();
  const { currentUser } = useApp();
  const { hasAuthority } = useAuthority();
  const [writeVisible, setWriteVisible] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState<any>(null);
  const [formValues, setFormValues] = useState({ title: '', subtitle: '', info: '' });
  ...
  const handleEdit = (record: any) => {
    ...
    setFormValues({ title: record?.title || '', subtitle: record?.subtitle || '', info: record?.info || '' });
    ...
    switch (record.status) {
      ...
      case 'FAIL':
        if (currentUser?.id === record.createUser) {
          ...
          actions.push(<Button size="small" key="delete" onClick={() => handleDelete(record)}>删除</Button>);
          actions.push(<Button size="small" key="submit" onClick={() => handleStatusChange(record, 'WAITING', null)}>提交审核</Button>);
        }
        break;
    }
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Typography variant="h5" sx={{ mb: 2 }}>网盘管理</Typography>
      <DataGridTable
        columns={columns}
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑网盘' : '新建网盘'}</DialogTitle>
        <DialogContent>
