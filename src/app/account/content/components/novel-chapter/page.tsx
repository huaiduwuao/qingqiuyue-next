...
import { page, remove, save, update } from '@/apis/content-novel-chapter';
...
const LIST_KEY = ['content', 'novel-chapter'];

export default function NovelChapterContentPage({ novel, handleClose }: Props) {
  const qc = useQueryClient();
  const [writeVisible, setWriteVisible] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState<any>(null);
  const [formValues, setFormValues] = useState({ num: '', name: '', url: '' });
  ...
  const handleEdit = (record: any) => {
    ...
    setFormValues({ num: record?.num || '', name: record?.name || '', url: record?.url || '' });
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h5">{novel?.title || '小说'} - 章节管理</Typography>
        <Button onClick={handleClose}>关闭</Button>
      </Box>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await page({ ...params, novelId: novel?.id });
