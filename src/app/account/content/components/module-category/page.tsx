...
import { page, save, update, remove, ModuleCategoryItem } from '@/apis/module-category';
import AddIcon from '@mui/icons-material/Add';

const LIST_KEY = ['content', 'module-category'];

interface Props {
  moduleId?: number;
}

export default function ModuleCategoryPage({ moduleId }: Props) {
  const qc = useQueryClient();
  const [modalVisible, setModalVisible] = useState(false);
  const [record, setRecord] = useState<ModuleCategoryItem | null>(null);
  ...
  const handleModalVisible = useCallback((flag: boolean, rec?: ModuleCategoryItem) => {
  ...
  const handleDelete = useCallback((row: ModuleCategoryItem) => {
    deleteMutation.mutate([row.id]);
  }, [deleteMutation]);

  const handleEdit = useCallback((row: ModuleCategoryItem) => {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 120,
      renderCell: (params) => (
        <img src={params.value} style={{ width: 60, height: 60, objectFit: 'cover' }} alt="" />
      ),
    },
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Typography variant="h6" sx={{ mb: 2 }}>分类管理</Typography>
      <DataGridTable
        columns={columns}
...
interface OperationModalProps {
  open: boolean;
  record: ModuleCategoryItem | null;
  onClose: () => void;
...
function OperationModal({ open, record, onClose, onSave, isSubmitting }: OperationModalProps) {
  ...
  React.useEffect(() => {
    if (record) {
      ...
      setValues({ title: '', subtitle: '', contentUrl: '', url: '', sort: 0 });
    }
  }, [record]);
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
          ...
          <TextField label="内容链接" value={values.contentUrl} onChange={handleChange('contentUrl')} fullWidth />
          <TextField label="图片URL" value={values.url} onChange={handleChange('url')} fullWidth />
