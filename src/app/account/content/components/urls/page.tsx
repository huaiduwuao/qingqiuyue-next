...
import { page, remove, save, update } from '@/apis/content-urls';
import AddIcon from '@mui/icons-material/Add';
import DeleteIcon from '@mui/icons-material/Delete';
import EditIcon from '@mui/icons-material/Edit';
import type { GridColDef } from '@mui/x-data-grid';

const LIST_KEY = ['content', 'urls'];

export default function UrlsContentPage() {
  const qc = useQueryClient();
  const [writeVisible, setWriteVisible] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState<any>(null);
  const [formValues, setFormValues] = useState({ title: '', url: '', type: '', method: '', parseDetail: '', content: '' });
  ...
  const handleEdit = (record: any) => {
    ...
    setFormValues({ title: record?.title || '', url: record?.url || '', type: record?.type || '', method: record?.method || '', parseDetail: record?.parseDetail || '', content: record?.content || '' });
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'url', headerName: '地址', width: 250 },
    { field: 'type', headerName: '类型', width: 100 },
    { field: 'method', headerName: '请求方法', width: 100 },
    { field: 'parseDetail', headerName: '解析详情', width: 100, renderCell: (params) => params.value === '1' ? '是' : '否' },
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Typography variant="h5" sx={{ mb: 2 }}>链接管理</Typography>
      <DataGridTable
        columns={columns}
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑链接' : '新建链接'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            <TextField label="标题" value={formValues.title} onChange={handleFormChange('title')} fullWidth />
            <TextField label="地址" value={formValues.url} onChange={handleFormChange('url')} fullWidth />
            <TextField label="类型" value={formValues.type} onChange={handleFormChange('type')} fullWidth />
            <TextField label="请求方法" value={formValues.method} onChange={handleFormChange('method')} fullWidth />
            <TextField label="解析详情" value={formValues.parseDetail} onChange={handleFormChange('parseDetail')} fullWidth />
            <TextField label="内容" value={formValues.content} onChange={handleFormChange('content')} fullWidth multiline rows={3} />
          </Box>
        </DialogContent>
        <DialogActions>
