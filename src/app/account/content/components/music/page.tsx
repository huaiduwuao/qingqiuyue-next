...
import { page, process, remove, save, update } from '@/apis/content-music';
...
export default function MusicContentPage() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'moduleContentStatus', headerName: '正式版', width: 120, renderCell: (params) => {
  ...
  return (
    <Box sx={{ height: "100%", overflow: "hidden" }}>
      <Typography variant="h5" sx={{ mb: 2 }}>音乐管理</Typography>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await page('music', { ...params, pageNumber: params.pageNumber });
          return { data: { records: res.data?.records || [], totalRow: res.data?.totalRow || 0 }, success: res.data?.success ?? true };
        }}
        onEdit={handleEdit}
        onDelete={handleDelete}
        toolBarRender={() => <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleEdit({})}>新建</Button>}
      />

      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑音乐' : '新建音乐'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
            <TextField label="标题" value={formValues.title} onChange={handleFormChange('title')} fullWidth />
            <TextField label="歌手" value={formValues.singer} onChange={handleFormChange('singer')} fullWidth />
