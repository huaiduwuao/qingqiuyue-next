...
import { page, remove, save, update } from '@/apis/system-dict-type';
import AddIcon from '@mui/icons-material/Add';
import DeleteIcon from '@mui/icons-material/Delete';
import EditIcon from '@mui/icons-material/Edit';
import type { GridColDef } from '@mui/x-data-grid';

const LIST_KEY = ['system', 'dict-type'];
...
export default function SystemDictTypePage() {
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 2 } }}>
      <Typography variant="h5" sx={{ mb: 2 }}>字典类型</Typography>
      <DataGridTable
        columns={columns}
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑字典类型' : '新建字典类型'}</DialogTitle>
        <DialogContent>
