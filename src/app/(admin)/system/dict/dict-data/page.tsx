...
import { list, remove, save, update } from '@/apis/system-dict-data';
...
const LIST_KEY = ['system', 'dict-data'];

export default function SystemDictDataPage({ typeId }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: (vals: any) => save({ ...vals, type: typeId }),
  ...
  const columns: GridColDef[] = [
    { field: 'value', headerName: '名称', width: 150 },
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 2 } }}>
      <Typography variant="h5" sx={{ mb: 2 }}>字典数据</Typography>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await list({ ...params, typeId });
          return { data: { records: res.data || [], totalRow: res.data?.totalRow || 0 }, success: res.data?.success ?? true };
        }}
        onEdit={handleEdit}
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑字典数据' : '新建字典数据'}</DialogTitle>
        <DialogContent>
