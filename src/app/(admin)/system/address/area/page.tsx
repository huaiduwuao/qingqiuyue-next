...
import { page, remove, save, update } from '@/apis/system-area';
import AddIcon from '@mui/icons-material/Add';
import DeleteIcon from '@mui/icons-material/Delete';
import EditIcon from '@mui/icons-material/Edit';
import type { GridColDef } from '@mui/x-data-grid';

const LIST_KEY = ['system', 'address-area'];
...
export default function SystemAddressAreaPage() {
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 2 } }}>
      <Typography variant="h5" sx={{ mb: 2 }}>区县管理</Typography>
      <DataGridTable
        columns={columns}
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑区县' : '新建区县'}</DialogTitle>
        <DialogContent>
