...
import { page, remove, save, update } from '@/apis/system-street';
import AddIcon from '@mui/icons-material/Add';
import DeleteIcon from '@mui/icons-material/Delete';
import EditIcon from '@mui/icons-material/Edit';
import type { GridColDef } from '@mui/x-data-grid';

const LIST_KEY = ['system', 'address-street'];
...
export default function SystemAddressStreetPage() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'areaId', headerName: '区县ID', width: 100 },
  ...
  return (
    <Box sx={{ p: { xs: 1.5, md: 2 } }}>
      <Typography variant="h5" sx={{ mb: 2 }}>街道管理</Typography>
      <DataGridTable
        columns={columns}
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑街道' : '新建街道'}</DialogTitle>
        <DialogContent>
