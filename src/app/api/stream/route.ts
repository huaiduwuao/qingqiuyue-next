 ...
 * 优先调后端通用解析引擎 /api/content/stream-parser/parse
 */
export async function GET(request: NextRequest) {
  ...
  // 2) 降级:本地默认解析器(与后端同一套配置驱动逻辑)
  return localParse(url);
}
...
async function localParse(url: string) {
  try {
    ...
    if (!matched) {
      return NextResponse.json({ code: 404, msg: '不支持的URL平台', data: { url } });
    }
    ...
    if (streams.length === 0) {
      return NextResponse.json({ code: 0, msg: '该视频可能需要VIP或无法解析', data: { url, platform: matched.platform, streams: [] } });
    }
    ...
    return NextResponse.json({ code: 500, msg: `解析失败: ${error instanceof Error ? error.message : '未知错误'}` }, { status: 500 });
  }
}
