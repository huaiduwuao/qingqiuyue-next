...
export default function MusicContentPage() {
  ...
  const handleSubmit = async () => {
    try {
      if (selectedRecord?.id) {
        await update('music', { ...selectedRecord, ...formValues });
      } else {
        await save('music', { ...formValues, status: 'DRAFT' });
      }
  ...
  const handleStatusChange = async (record: any, status: string | null, moduleContentStatus: string | null) => {
    try {
      await process('music', { ids: [record.id], status, moduleContentStatus });
      showMessage('操作成功');
    } catch (err: any) {
      showMessage(err.message || '操作失败', 'error');
    }
