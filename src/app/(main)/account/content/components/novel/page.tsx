...
export default function NovelContentPage() {
  ...
  const handleSubmit = async () => {
    try {
      if (selectedRecord?.id) {
        await update('novel', { ...selectedRecord, ...formValues });
      } else {
        await save('novel', { ...formValues, status: 'DRAFT' });
      }
  ...
  const handleDelete = async (record: any) => {
    ...
    try { await remove('novel', [record.id]); showMessage('删除成功'); } catch (err: any) { showMessage(err.message || '删除失败', 'error'); }
  };

  const handleStatusChange = async (record: any, status: string | null, moduleContentStatus: string | null) => {
    try { await process('novel', { ids: [record.id], status, moduleContentStatus }); showMessage('操作成功'); } catch (err: any) { showMessage(err.message || '操作失败', 'error'); }
  };
