...
export default function FilmContentPage() {
  ...
  const handleSubmit = async () => {
    try {
      if (selectedRecord?.id) {
        await update('film', { ...selectedRecord, ...formValues });
      } else {
        await save('film', { ...formValues, status: 'DRAFT' });
      }
  ...
  const handleDelete = async (record: any) => {
    ...
    try { await remove('film', [record.id]); showMessage('删除成功'); } catch (err: any) { showMessage(err.message || '删除失败', 'error'); }
  };

  const handleStatusChange = async (record: any, status: string | null, moduleContentStatus: string | null) => {
    try { await process('film', { ids: [record.id], status, moduleContentStatus }); showMessage('操作成功'); } catch (err: any) { showMessage(err.message || '操作失败', 'error'); }
  };
