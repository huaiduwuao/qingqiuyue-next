...
export default function SystemMenuPage() {
  ...
  const handleSubmit = async () => {
    try {
      if (selectedRecord?.id) {
        await update(selectedRecord.id, formValues);
        showMessage('更新成功');
      } else {
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Typography variant="h5" sx={{ mb: 2 }}>菜单管理</Typography>
      <DataGridTable
        columns={columns}
        fetchData={async (params) => {
          const res = await list(params);
          return { data: { records: res.data || [], totalRow: res.data?.length || 0 }, success: res.data?.success ?? true };
        }}
        onEdit={handleEdit}
      ...
      <Dialog open={writeVisible} onClose={() => setWriteVisible(false)} maxWidth="md" fullWidth>
        <DialogTitle>{selectedRecord?.id ? '编辑菜单' : '新建菜单'}</DialogTitle>
        <DialogContent>
