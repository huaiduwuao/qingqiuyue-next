...
export async function getWxMpStatus(): Promise<WxMpStatus> {
  return adminClient<WxMpStatus>('/admin/wx/mp/status');
}
...
export async function getWxMpConfig(): Promise<Record<WxAppKind, WxAppConfig>> {
  return adminClient('/admin/wx/mp/config');
}
...
export async function getWxMenu(): Promise<{ buttons: WxMenuNode[]; message?: string }> {
  return adminClient('/admin/wx/mp/menu');
}
...
export async function syncWxFollowers(): Promise<{ synced: number }> {
  return adminClient.post('/admin/wx/mp/user/sync', {});
}
